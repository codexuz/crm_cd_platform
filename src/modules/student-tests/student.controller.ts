...
export class StudentController {
  ...
  @Post('check-listening')
  @ApiOperation({
    summary: 'Check listening answers',
    description:
      'Automatically checks listening answers against correct answer keys and calculates score',
  })
  @ApiResponse({
    status: 200,
    description: 'Listening answers checked successfully',
    schema: {
      type: 'object',
      properties: {
        correct: { type: 'number', example: 27 },
        incorrect: { type: 'number', example: 13 },
        score: { type: 'number', example: 6.5 },
        totalQuestions: { type: 'number', example: 40 },
      },
    },
  })
  @ApiResponse({
    status: 400,
    description: 'No listening answers found',
  })
  async checkListening(@GetStudent('candidateId') candidateId: string) {
  ...
  @Post('check-reading')
  @ApiOperation({
    summary: 'Check reading answers',
    description:
      'Automatically checks reading answers against correct answer keys and calculates score',
  })
  @ApiResponse({
    status: 200,
    description: 'Reading answers checked successfully',
    schema: {
      type: 'object',
      properties: {
        correct: { type: 'number', example: 30 },
        incorrect: { type: 'number', example: 10 },
  ...
  @ApiResponse({
    status: 400,
    description: 'No reading answers found',
  })
  async checkReading(@GetStudent('candidateId') candidateId: string) {
    return await this.studentTestsService.checkReadingAnswers(candidateId);
  }

  // Grade writing section (manual)
  @Post('check-writing')
  @ApiOperation({
    summary: 'Grade writing section',
    description: 'Manually grade writing tasks and provide scores and feedback',
  })
  @ApiBody({
    schema: {
      type: 'object',
      properties: {
        task1Score: { type: 'number', example: 6.5 },
        task2Score: { type: 'number', example: 7.0 },
        feedback: {
          type: 'string',
          example: 'Good structure, needs more vocabulary variety',
        },
      },
    },
  })
  @ApiResponse({
    status: 200,
    description: 'Writing section graded successfully',
    schema: {
      type: 'object',
      properties: {
        task1Score: { type: 'number', example: 6.5 },
        task2Score: { type: 'number', example: 7.0 },
        averageScore: { type: 'number', example: 6.75 },
        feedback: { type: 'string', example: 'Good structure' },
      },
    },
  })
  @ApiResponse({
    status: 400,
    description: 'No writing answers found',
  })
  async checkWriting(
    @GetStudent('candidateId') candidateId: string,
    @Body()
  ...
  // Auto-grade entire test
  @Post('auto-grade')
  @ApiOperation({
    summary: 'Auto-grade entire test',
    description:
      'Automatically grades listening and reading sections. Writing requires manual grading.',
  })
  @ApiResponse({
    status: 200,
    description: 'Test auto-graded successfully',
    schema: {
      type: 'object',
      properties: {
        listening: {
  ...
  async autoGrade(@GetStudent('candidateId') candidateId: string) {
